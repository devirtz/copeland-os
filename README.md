...
graph TD
    ...
    subgraph QEMU
        WIN["Windows Guest VM (undetected)"]
    end

    NET["Network VM (isolated)"]
    INTERNET[Internet]

    CopelandOS -->|"launches"| QEMU
    CopelandOS -->|"all traffic forced through"| NET
    WIN -->|"traffic routed via host"| CopelandOS
    NET --> INTERNET
    INTERNET -.->|"real IP hidden"| NET
```
...
⣇⢾⡱⠞⠈⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣸⣿⣿⡇⠀⠀⠀⠉⠛⠳⠿⣶⣽⣿⣿⣿⣿⣿⣿⣿⣿
```
Abandons the physical flesh for a purely digital existence